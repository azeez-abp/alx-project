...
class APIDocsView(MethodView):
    ...
    def get(self):
        ...
        urls = [
            {"name": spec["name"], "url": spec["url"]} for spec in specs if spec["name"]
        ]
        data = {
            "specs": specs,
            "urls": urls,
        ...
        if request.args.get("json"):
            ...
            data["favicon"] = self.config.get(
                "favicon", url_for("flasgger.static", filename="favicon-32x32.png")
            )
            ...
            data["swagger_ui_standalone_preset_js"] = self.config.get(
                ...
                url_for("flasgger.static", filename="swagger-ui-standalone-preset.js"),
            )
            data["jquery_js"] = self.config.get(
                "jquery_js", url_for("flasgger.static", filename="lib/jquery.min.js")
            )
            data["swagger_ui_css"] = self.config.get(
                "swagger_ui_css", url_for("flasgger.static", filename="swagger-ui.css")
            )
            return render_template("flasgger/index.html", **data)
...
class Swagger(object):
    ...
    def register_views(self, app):
        ...
        if self.config.get("swagger_ui", True):
            ...
            blueprint = Blueprint(self.config.get("endpoint", "flasgger"), __name__)
